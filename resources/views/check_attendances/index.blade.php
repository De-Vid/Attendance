        ...
        <div class="card-body p-0">
            ...
            <div class="table-responsive d-none d-md-block">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        {{-- SHOW LEAVE RECORDS FIRST (for general view only) --}}
                        @if(!request()->has('employee_id') && isset($leavesOnLeave) && count($leavesOnLeave) > 0)
                            @foreach($leavesOnLeave as $leave)
                            <tr class="table-success" style="background-color: #fff3cd;">
                                ...
                                <td>
                                    <div class="d-flex align-items-center justify-content-between">
                                        <div class="d-flex align-items-center">
                                            <div class="avatar-placeholder me-3 bg-primary-subtle">
            ...
            <div class="d-block d-md-none p-3">
                {{-- SHOW LEAVE RECORDS FIRST FOR MOBILE --}}
                @if(!request()->has('employee_id') && isset($leavesOnLeave) && count($leavesOnLeave) > 0)
                    @foreach($leavesOnLeave as $leave)
                    <div class="card border border-warning-subtle shadow-sm rounded-3 mb-3 p-3" style="background-color: #fff3cd;">
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <div class="d-flex align-items-center">
                                <div class="avatar-placeholder me-3 bg-warning" style="width: 42px; height: 42px; display: flex; align-items: center; justify-content: center;">
                                    {{ mb_substr($leave->user->name ?? 'U', 0, 1) }}
                                </div>
                                <div>
                                    <h6 class="mb-0 fw-bold text-dark">
                                        {{ $leave->user->name ?? 'មិនស្គាល់' }}
                                        <span class="badge bg-warning ms-1" style="font-size: 9px;">ច្បាប់</span>
                                    </h6>
                                    <small class="text-muted d-block">{{ $leave->user->email ?? '' }}</small>
                                    <small class="text-secondary fw-semibold">ID: #{{ $leave->user->employee->staff_id ?? 'N/A' }}</small>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row g-2 mb-3 text-start">
                            <div class="col-12">
                                <small class="text-muted d-block">រយៈពេលច្បាប់</small>
                            ...
                            <div class="col-12">
                                <small class="text-muted d-block">ប្រភេទច្បាប់</small>
                                <span class="text-dark fw-medium small">
                                    @switch($leave->type)
                                        @case('annual') 📅 ច្បាប់ប្រចាំឆ្នាំ @break
                                        @case('sick') 🤒 ច្បាប់ឈឺ @break
                        ...
                        <div class="d-flex justify-content-between align-items-center pt-1">
                            <span class="badge bg-warning text-dark px-3 py-2 rounded-pill fw-semibold">
                                <i class="bi bi-check-circle me-1"></i> បានអនុញ្ញាត
                            </span>
                            <span class="text-muted small">អវត្តមាន</span>
                        </div>
                    </div>
                    @endforeach
                @endif

                {{-- SHOW ATTENDANCE RECORDS FOR MOBILE --}}
                @forelse($attendances as $attendance)
...
/* Additional styles for leave records */
.bg-warning-subtle {
